...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, register_blk=-1, top_k: int = 0):
        B = x.shape[0]
        ...
        if top_k is not None and top_k > 0:
            ...
            def _zero_topk_patch_grads(grad):
                """
                grad: [B, L, C], gradients of patch embeddings before transformer.
                """
                grad_norm = grad.norm(p=2, dim=-1)  # [B, L]
                patch_norms = grad_norm[:, 1:]      # exclude CLS token
                k = min(top_k, patch_norms.shape[1]) if patch_norms.shape[1] > 0 else 0
                if k == 0:
                    return grad

                # Get indices of top-k largest norms per sample
                _, topk_idx = torch.topk(patch_norms, k=k, dim=1, largest=True, sorted=False)

                # Build mask: 1 for keep, 0 for zeroed
                mask_patches = torch.ones_like(patch_norms)
                mask_patches.scatter_(1, topk_idx, 0.0)

                cls_mask = torch.ones((grad.shape[0], 1), device=grad.device, dtype=grad.dtype)
                full_mask = torch.cat([cls_mask, mask_patches], dim=1).unsqueeze(-1)  # [B, L, 1]

                return grad * full_mask  # zero out top-k patches
