...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None):
        super().__init__()
        self.width = width
        self.layers = layers # 12 for B-32
        self.resblocks = nn.Sequential(*[ResidualAttentionBlock(width, heads, attn_mask) for _ in range(layers)])

    def forward(self, x: torch.Tensor, output_layers: list = None):
        if output_layers is not None:
            output_layers = sorted(set(output_layers))
            layer_outputs = {}
            for i, block in enumerate(self.resblocks, start=1):
                x = block(x)
                if i+1 in output_layers:
                    layer_outputs[i+1] = x.permute(1, 0, 2)[:,1:,:].clone()  # [B, L, D]
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, output_layers: list = None, top_k: int = 0):
        ...
        if output_layers is not None: 
            x, layers_embds = self.transformer(x, output_layers)
        else:
            x = self.transformer(x)

        x = x.permute(1, 0, 2)  # [B, L, C]

        # layernorm post-transformer
        x = self.ln_post(x[:, 0, :])

        cls_emb = x @ self.proj

        if output_layers is not None:
            return cls_emb, layers_embds
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, output_layers: list = None):
        if isinstance(self.visual, VisionTransformer):
            return self.visual(image.type(self.dtype), output_layers=output_layers)
        else:
