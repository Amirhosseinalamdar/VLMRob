...
# modeling_outputs are stable; keep if you need the types elsewhere
...
# PreTrainedModel is fine; SequenceSummary may not exist in newer releases.
try:
    from transformers.modeling_utils import PreTrainedModel  # noqa: F401
except Exception:
    PreTrainedModel = object  # minimal fallback; not used below

# Conv1D and prune utilities moved around across versions; guard them.
try:
    ...
    try:
        # older/newer GPT-2 module still exposes these
        from transformers.models.gpt2.modeling_gpt2 import (
            Conv1D, find_pruneable_heads_and_indices, prune_conv1d_layer
        )  # noqa: F401
    except Exception:
        # define very small shims if nothing is available (they're not used in this file)
        Conv1D = None
        def find_pruneable_heads_and_indices(*args, **kwargs):  # noqa: E306
            return [], None
        def prune_conv1d_layer(*args, **kwargs):
            return args[0]

from transformers.utils import ModelOutput, logging
# model_parallel_utils sometimes moves; guard it.
try:
    from transformers.utils.model_parallel_utils import assert_device_map, get_device_map  # noqa: F401
except Exception:
