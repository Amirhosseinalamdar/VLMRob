...
# PreTrainedModel is still in modeling_utils; SequenceSummary may not be.
from transformers.modeling_utils import PreTrainedModel
try:
    # old Transformers
    from transformers.modeling_utils import SequenceSummary  # noqa: F401
except Exception:
    try:
        # some versions re-expose it under GPT-2
        from transformers.models.gpt2.modeling_gpt2 import SequenceSummary  # noqa: F401
    except Exception:
        # define a minimal no-op fallback (not used by this file, but keeps imports happy)
        import torch
        from torch import nn
        class SequenceSummary(nn.Module):  # type: ignore
            ...
            def forward(self, hidden_states, *args, **kwargs):
                # mimic common behavior: take last token if [B, T, H]
                if hasattr(hidden_states, "dim") and hidden_states.dim() == 3:
                    hidden_states = hidden_states[:, -1, :]
                return hidden_states

# Conv1D also moved around in some releases
try:
