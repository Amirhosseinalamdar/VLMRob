...
class SmallCap(PreTrainedModel):
    ...
    def get_input_embeddings(self):
        """Get the model's input embeddings for compatibility with newer Transformers"""
        if hasattr(self, 'decoder') and hasattr(self.decoder, 'get_input_embeddings'):
    ...
    # ):
    #     """
    #     Generates captions for the given image pixels.
        
    #     This method implements beam search/text generation for the SmallCap model.
    #     """
    ...
    #         decoder_input_ids = torch.tensor([[self.config.decoder_start_token_id]] * batch_size).to(pixel_values.device)
    ...
    #     # Use the decoder's generate method if it exists, otherwise implement custom generation
    #     if hasattr(self.decoder, 'generate'):
    #         # If decoder has generate method, use it with encoder outputs
    ...
    #         )
    #     else:
    #         # Manual implementation of greedy search/beam search
    #         return self._generate_manual(
    ...
    # ):
    #     """
    #     Manual implementation of text generation for models without built-in generate.
    #     """
    ...
    #             input_ids=sequences[:, -1:] if sequences.shape[1] > 1 else sequences,
    ...
    #             next_tokens = torch.topk(next_token_probs, num_beams, dim=-1).indices
    #         else:
    #             # Greedy decoding
    ...
    #             active = active & ~eos_mask.squeeze(-1)
