...
from transformers.models.gpt2.modeling_gpt2 import (
    ...
    GPT2LMHeadModel, GPT2MLP, GPT2Attention, GPT2Block, GPT2Model,
)

# OPT pieces (some releases don’t export OPTDecoder)
try:
    from transformers.models.opt.modeling_opt import (
        OPTForCausalLM, OPTAttention, OPTDecoderLayer, OPTModel, OPTDecoder,
    )
except Exception:
    from transformers.models.opt.modeling_opt import (
        OPTForCausalLM, OPTAttention, OPTDecoderLayer, OPTModel,
    )
    OPTDecoder = None  # not used in many forks; keeps import from failing

from transformers.activations import ACT2FN

# Consolidate modeling_outputs imports (avoid duplicate blocks)
...
# PreTrainedModel is stable; SequenceSummary is not always exported — shim it.
from transformers.modeling_utils import PreTrainedModel
try:
    from transformers.modeling_utils import SequenceSummary  # older TF exposes it here
except Exception:
    try:
        from transformers.models.gpt2.modeling_gpt2 import SequenceSummary  # some TF versions re-export it here
    except Exception:
        # minimal fallback (unused by this file, but avoids ImportError)
        class SequenceSummary(nn.Module):  # type: ignore
            def __init__(self, config=None):
                super().__init__()
                self.summary = nn.Identity()
            def forward(self, hidden_states, *args, **kwargs):
                if hasattr(hidden_states, "dim") and hidden_states.dim() == 3:
                    hidden_states = hidden_states[:, -1, :]
                return hidden_states

# Conv1D/prune utils moved around across versions; guard them.
try:
...
# model_parallel_utils sometimes moves; provide no-op fallbacks if missing.
try:
    from transformers.utils.model_parallel_utils import assert_device_map, get_device_map
except Exception:
    def assert_device_map(*args, **kwargs):  # type: ignore
        return True
    def get_device_map(*args, **kwargs):  # type: ignore
        return {}
